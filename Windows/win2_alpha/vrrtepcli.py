...
def getnavid(wordin):
	...
	# Changed wordin to uin for unicode support    
        antiprefixedNoun,prefixes = root.antiprefix(uin)
        if DEBUG:
        ...
        for line in metaWords:
	   # No tabs here. They're still attached from the initial failed attempt to find the word!
	   # Also, it's still unicode fixed, so no ned to redo that either
	    inword = antilen
